...
/* =====================================================================
   MESTRE – 5-MINUTOS
   • Uma linha por amostra de 5 min da estação Davis                     (UPDATE)
   • Dados horários (INMET + OpenWeather Now / Forecast / Air) acoplados
     pela chave ts_hour – hora cheia no horário de Brasília (UTC-3)
   ===================================================================== */
/* =======================================================================
   MESTRE – 5 MIN
   Cria (ou recria) wherehouse_tratado.mestre_5min
   =======================================================================*/

-- Create table versão 3

CREATE OR REPLACE TABLE
  `local-bliss-359814.wherehouse_tratado.mestre_5min`
AS
WITH
/* ───────────────────────── 1. UPDATE – 5 min ───────────────────────── */
upd_5min AS (
  SELECT
    CAST(timestamp AS STRING)                                         AS timestamp_br,
    CAST(TIMESTAMP_TRUNC(TIMESTAMP(timestamp), HOUR) AS STRING)       AS ts_hour,
    ...
    CAST(ventodirecao           AS STRING)  AS ventodirecao,
    CAST(status                 AS STRING)  AS status,
    CAST(data_station_davis     AS STRING)  AS data_station_davis,
    ...
    CAST(data_lua               AS STRING)  AS data_lua,
    CAST(nascer_do_sol          AS STRING)  AS nascer_do_sol,
    CAST(por_do_sol             AS STRING)  AS por_do_sol,
    CAST(matutino               AS STRING)  AS matutino,
    CAST(vespertino             AS STRING)  AS vespertino,
    CAST(tipo_mare_getControls  AS STRING)  AS tipo_mare_getcontrols,
    CAST(data_mare_getControls  AS STRING)  AS data_mare_getcontrols,
    CAST(data_hidromares        AS STRING)  AS data_hidromares,
    CAST(data_mare_getMare      AS STRING)  AS data_mare_getmare,
    CAST(data_mare_real_getMare AS STRING)  AS data_mare_real_getmare,
    CAST(api_mare               AS STRING)  AS api_mare,
    CAST(api_hidromares         AS STRING)  AS api_hidromares,
    CAST(api_estatistica        AS STRING)  AS api_estatistica,
    CAST(motivo                 AS STRING)  AS motivo
  FROM `local-bliss-359814.wherehouse.update_rawdata_5min_backfill`
),

/* ─────────────── 2. INMET – mantém só o último registro/h ─────────────── */
inm_hour AS (
  SELECT
    CAST(
      TIMESTAMP_TRUNC(
    ...
    CAST(TIMESTAMP_SUB(TIMESTAMP(dt_utc), INTERVAL 3 HOUR) AS STRING) AS inmet_datetime,
...
/* ─────────────── 3. OpenWeather NOW – último por hora ─────────────── */
ow_now AS (
  SELECT
    CAST(
      TIMESTAMP_TRUNC(
        TIMESTAMP_SUB(TIMESTAMP(timestamp_utc), INTERVAL 3 HOUR), HOUR) AS STRING
    ) AS ts_hour,
    CAST(TIMESTAMP_SUB(MAX(TIMESTAMP(timestamp_utc)), INTERVAL 3 HOUR) AS STRING)
...
/* ─────────── 4. Limite: última hora com dado real ─────────── */
limite_hora_real AS (
  SELECT MAX(TIMESTAMP_TRUNC(TIMESTAMP(timestamp), HOUR)) AS ts_limite
  FROM `local-bliss-359814.wherehouse.update_rawdata_5min_backfill`
),

/* ───── 5. Forecast – replica 1 h, mas só até ts_limite ───── */
fc_rep AS (
  SELECT
    CAST(
      TIMESTAMP_TRUNC(
  ...
  WHERE city_name = 'Rio Grande'
    AND TIMESTAMP_TRUNC(TIMESTAMP(timestamp_execucao), HOUR)
        <= limite_hora_real.ts_limite
  GROUP BY ts_hour
),

/* ─── 6. Air-Pollution – lógica idêntica ─── */
air_rep AS (
  SELECT
    CAST(
      TIMESTAMP_TRUNC(
...
SELECT
  u.timestamp_br,
  u.ts_hour,
  -- todos os campos da Davis (upd_5min), exceto timestamp_br e ts_hour
  ...
  u.tipo_mare_getcontrols, u.data_mare_getcontrols, u.data_hidromares, u.data_mare_getmare,
  u.data_mare_real_getmare, u.api_mare, u.api_hidromares, u.api_estatistica, u.motivo,
...
fc_rep AS (
  SELECT
    ...
    SAFE_CAST(ANY_VALUE(temp) AS NUMERIC) AS fc_temp,
    SAFE_CAST(ANY_VALUE(pop) AS NUMERIC) AS fc_pop,
    SAFE_CAST(ANY_VALUE(feels_like) AS NUMERIC) AS fc_feels_like,
    CAST(ANY_VALUE(weather_main) AS STRING) AS fc_weather_main,
    CAST(ANY_VALUE(weather_description) AS STRING) AS fc_weather_desc
  FROM `local-bliss-359814.wherehouse.dados_openweather_forecast`
  CROSS JOIN limite_hora_real
  WHERE city_name = 'Rio Grande'
    AND TIMESTAMP_TRUNC(TIMESTAMP(timestamp_execucao), HOUR)
...
-- Create table versão 1

CREATE OR REPLACE TABLE `local-bliss-359814.wherehouse_tratado.mestre_5min` AS
WITH
-- 1. Base principal (5 min)
upd_5min AS (
  SELECT
    TIMESTAMP_SECONDS(DIV(UNIX_SECONDS(TIMESTAMP(timestamp)), 300) * 300) AS ts_5min,
    timestamp AS timestamp_br,
    *
  FROM `local-bliss-359814.wherehouse.update_rawdata_5min_backfill`
),
...
inmet AS (
  SELECT
    TIMESTAMP_TRUNC(TIMESTAMP(timestamp_execucao), HOUR) AS ts_hour_inmet,
    timestamp_execucao AS inmet_timestamp_execucao,
    temperatura_inst AS temperatura_inst_inmet,
    temperatura_max AS temperatura_max_inmet,
    temperatura_min AS temperatura_min_inmet,
    umidade_inst AS umidade_inst_inmet,
    pressao_inst AS pressao_inst_inmet,
    vento_vel_m_s AS vento_vel_m_s_inmet,
    vento_dir_deg AS vento_dir_deg_inmet,
    chuva AS chuva_inmet,
    radiacao AS radiacao_inmet
...
ow_now AS (
  SELECT
    TIMESTAMP_SECONDS(DIV(UNIX_SECONDS(TIMESTAMP(timestamp)), 300) * 300) AS ts_5min_ow,
    timestamp AS openweather_timestamp,
    main_temp AS ow_temp,
    main_humidity AS ow_humidity,
    wind_speed AS ow_wind_speed,
    clouds_all AS ow_clouds,
    weather_main AS ow_weather_main,
    weather_description AS ow_weather_desc
  FROM `local-bliss-359814.wherehouse.dados_openwhather`
  WHERE city = 'Rio Grande'
),

-- 4. OpenWeather FORECAST (3h)
fc AS (
  SELECT
    TIMESTAMP_TRUNC(TIMESTAMP(timestamp_execucao), HOUR) AS ts_hour_fc,
    timestamp_execucao AS forecast_timestamp_execucao,
    temp AS fc_temp,
    pop AS fc_pop,
    feels_like AS fc_feels_like,
    weather_main AS fc_weather_main,
    weather_description AS fc_weather_desc
  FROM `local-bliss-359814.wherehouse.dados_openweather_forecast`
  WHERE city_name = 'Rio Grande'
),

-- 5. OpenWeather AIR POLLUTION (3h)
air AS (
  SELECT
    TIMESTAMP_TRUNC(TIMESTAMP(timestamp_execucao), HOUR) AS ts_hour_air,
    timestamp_execucao AS airpollution_timestamp_execucao,
    aqi AS air_aqi,
    pm2_5 AS air_pm2_5,
    pm10 AS air_pm10,
    co AS air_co,
...
SELECT
  u.ts_5min AS timestamp_br,
  u.* EXCEPT(ts_5min, timestamp_br),

  -- INMET (match pela hora)
  i.inmet_timestamp_execucao,
  i.temperatura_inst_inmet, i.temperatura_max_inmet, i.temperatura_min_inmet,
  i.umidade_inst_inmet, i.pressao_inst_inmet, i.vento_vel_m_s_inmet,
  i.vento_dir_deg_inmet, i.chuva_inmet, i.radiacao_inmet,

  -- OpenWeather NOW (match por 5 min)
  o.openweather_timestamp, o.ow_temp, o.ow_humidity, o.ow_wind_speed,
  o.ow_clouds, o.ow_weather_main, o.ow_weather_desc,

  -- Forecast (match pela hora)
  f.forecast_timestamp_execucao, f.fc_temp, f.fc_pop, f.fc_feels_like,
  f.fc_weather_main, f.fc_weather_desc,

  -- Air Pollution (match pela hora)
  a.airpollution_timestamp_execucao, a.air_aqi, a.air_pm2_5, a.air_pm10,
  a.air_co, a.air_no, a.air_no2, a.air_o3, a.air_so2, a.air_nh3

FROM upd_5min u

LEFT JOIN inmet i
  ON TIMESTAMP_TRUNC(TIMESTAMP(u.timestamp_br), HOUR) = i.ts_hour_inmet

LEFT JOIN ow_now o
  ON u.ts_5min = o.ts_5min_ow

LEFT JOIN fc f
  ON TIMESTAMP_TRUNC(TIMESTAMP(u.timestamp_br), HOUR) = f.ts_hour_fc

LEFT JOIN air a
  ON TIMESTAMP_TRUNC(TIMESTAMP(u.timestamp_br), HOUR) = a.ts_hour_air
