...
ow_now AS (
  SELECT
   ...
   SAFE_CAST(ANY_VALUE(weather_id)     AS NUMERIC) AS ow_weather_id,
    SAFE_CAST(ANY_VALUE(weather_main)        AS STRING)  AS ow_weather_main,
    SAFE_CAST(ANY_VALUE(weather_description) AS STRING)  AS ow_weather_desc,
    SAFE_CAST(ANY_VALUE(weather_icon)        AS STRING)  AS ow_weather_icon,
    SAFE_CAST(ANY_VALUE(base)                AS STRING)  AS ow_base,
   ...
   SAFE_CAST(ANY_VALUE(sys_id)         AS NUMERIC) AS ow_sys_id,
    SAFE_CAST(ANY_VALUE(sys_country)         AS STRING)  AS ow_sys_country,
...
fc_rep AS (
  SELECT
    SAFE_CAST(TIMESTAMP_TRUNC(TIMESTAMP(timestamp_execucao), HOUR) AS STRING) AS ts_hour,
...
air_rep AS (
  SELECT
    SAFE_CAST(TIMESTAMP_TRUNC(TIMESTAMP(timestamp_execucao), HOUR) AS STRING) AS ts_hour,
    SAFE_CAST(MAX(timestamp_execucao) AS STRING) AS airpollution_timestamp_execucao,
  ...
  inicio_update AS (
    SELECT MIN(ts_hour) AS ts_hour_min
    FROM upd_hour
  ),
horas AS (
